...
if type(getgenv().Configs) ~= "table" then
    getgenv().Configs = {
		...
		["SecondSea"] = {
			...
			["AutoFactory"] = true,
			["AutoThirdSea"] = true,
			["AlliesFruit"] = {"Dragon-Dragon","Spirit-Spirit","Venom-Venom","Dough-Dough","Kitsune-Kitsune"},
		},
		["ThirdSea"] = {
			-- Sea 3
		...
		["Fps"] = {
			...
			["FpsBoost"] = true, -- false/true
			["LockFPS"] = 60,
			["LockFPSNow"] = true, -- false/true
			["WhiteScreen"] = false, -- false/true 
		}
	}
end
...
local sitinklib = loadstring(game:HttpGet("https://raw.githubusercontent.com/NguyenLam2504/pearcathub.lua/main/Gui_For_Skidder.lua%3Fraw%3Dtrue"))()
...
["Content"] = "Welcome to Pear Cat Hub",
["Time"] = 1,
["Delay"] = 10
})
...
["Logo Info"] = "rbxassetid://18129421604",
["Logo Player"] = "rbxassetid://18129421604",
["Name Info"] = "Pear Cat Hub Info",
["Name Player"] = "FumoUser",
["Info Description"] = "discord.gg/9PzEWrzgXR",
["Tab Width"] = 135,
["Color"] = Color3.fromRGB(127.00000002980232, 146.00000649690628, 242.00000077486038),
...
sitinklib:Notify({
            ...
            ["Description"] = "",
            ["Color"] = Color3.fromRGB(127.00000002980232, 146.00000649690628, 242.00000077486038),
            ["Content"] = "Close UI...! Double Click To Show UI...!",
            ["Time"] = 1,
            ["Delay"] = 10
        })
end
...
local doi_sivi = Section1:Button({
    ...
    ["Content"] = "",
    ["Callback"] = function()
        loadstring(game:HttpGet("https://raw.githubusercontent.com/NguyenLam2504/PhantomHub/main/HopServer.lua"))()
    end
})
--  loadstring(game:HttpGet("https://raw.githubusercontent.com/NguyenLam2504/LunarXHubNextGeneration/main/youaremysunshie.lua"))()
local joib_id = Section1:TextInput({
    ["Title"] = "Job Id",
    ["Content"] = "",
    ["Place Holder Text"] = "Enter your job id here...",
...
function KaiTunTool:WebHookSend(WebHookUrl)
	local Embeds = {{
		title = ":sushi: **    Fumo Hub Kaitun Status    **",
...
LPH_NO_VIRTUALIZE(function()
	...
	gg.__index = newcclosure(function(...)
		...
		if StartKaiTun then
			...
			elseif OldWorld and args[1] == MouseCheckReq and args[2] == "Hit" and not checkcaller() and inmyself('Slingshot') and UsefastattackPlayers == false and StatusFarm:Set({["Title"]="Status Farm",["Content"]="Skip Farm Level"}) then
...
function AutoKaiTunOldWorld(i1,i2,i3,i4)
	...
	if AutoPole and MyLevel >= 200 and (game.ReplicatedStorage:FindFirstChild("Thunder God [Lv. 575] [Boss]") or game.Workspace.Enemies:FindFirstChild("Thunder God [Lv. 575] [Boss]")) then
        StatusFarm:Set({
            ["Title"] = "Status Farm",
            ["Content"] = "Kill Thunder God"
        })
	...
	elseif AutoSaber and MyLevel >= 200 and game.Workspace.Map.Jungle.Final.Part.CanCollide == false and (game.ReplicatedStorage:FindFirstChild("Saber Expert [Lv. 200] [Boss]") or game.Workspace.Enemies:FindFirstChild("Saber Expert [Lv. 200] [Boss]")) then
        StatusFarm:Set({
            ["Title"] = "Status Farm",
            ["Content"] = "Kill Saber"
        })
	...
	elseif AutoSea2 and MyLevel >= 700 then
        StatusFarm:Set({
            ["Title"] = "Status Farm",
            ["Content"] = "Doing Quest Sea 2"
        })
...
function AutoKaiTunNewWorld(i1,i2,i3,i4)
	...
	local MyLevel = i3
	local CheckMyStatusOfDF = i4
	if (AutoDeathStep and game.Players.LocalPlayer.PlayerGui.Main.Timer.Visible == false and Com("F_","BuyDeathStep",true) ~= 1 and game:GetService("Workspace").Map.IceCastle.Hall.LibraryDoor:FindFirstChild("PhoeyuDoor")) and (game.ReplicatedStorage:FindFirstChild("Awakened Ice Admiral [Lv. 1400] [Boss]") or game.Workspace.Enemies:FindFirstChild("Awakened Ice Admiral [Lv. 1400] [Boss]")) then
		StatusFarm:Set({
			["Title"] = "Status Farm",
			["Content"] = "Kill Awaken Ice"
		})
	...
	elseif game:GetService("Workspace").Map.IceCastle:FindFirstChild("RengokuChest") and game.Players.LocalPlayer.PlayerGui.Main.Timer.Visible == false and inmyself("Hidden Key") and AutoRengoku then 
        StatusFarm:Set({
            ["Title"] = "Status Farm",
            ["Content"] = "Use Hidden Key"
        })
	...
	elseif game:GetService("Workspace").Map.IceCastle.Hall.LibraryDoor:FindFirstChild("PhoeyuDoor") and inmyself("Library Key") and AutoDeathStep and NewWorld and Com("F_","OpenLibrary",true) ~= true then
        StatusFarm:Set({
            ["Title"] = "Status Farm",
            ["Content"] = "Use Library Key"
        })
		EquipWeapon("Library Key")
		task.wait(0.1)
		Com("F_","OpenLibrary")
	elseif MyLevel >= 1500 and game.Players.LocalPlayer.PlayerGui.Main.Timer.Visible == false and AutoRengoku then
        StatusFarm:Set({
            ["Title"] = "Status Farm",
            ["Content"] = "Do Rengoku"
        })
	...
	elseif MyLevel >= 1500 and AutoSea3 and not SkipGetItemSoulInSea2 and AutoSoulGuitar and game.Players.LocalPlayer.PlayerGui.Main.Timer.Visible == false and ((GetMaterial("Dark Fragment") < 1 and (function()for a,b in pairs(game.Workspace:GetDescendants())do if b.Name:match("Chest")and b:IsA("BasePart")then return true end end;return false end)()) or GetMaterial("Ectoplasm") < 300) then
        StatusFarm:Set({
            ["Title"] = "Status Farm",
            ["Content"] = "DO Soul Gitar"
        })
...
function AutoKaiTunThreeWorld(i1,i2,i3,i4)
	...
	elseif AutoCDK and 
		MyLevel >= 2200 and GetMasteryWeaponOnline("Yama") >= 350 and GetMasteryWeaponOnline("Tushita") >= 350 and CDKCheckAllIn1("done") == false and
        ...
        StatusFarm:Set({
            ["Title"] = "Status Farm",
            ["Content"] = "Do CDK ( Cursed Dual Katana )"
        })
		local CheckPQuest
		if game:GetService("Workspace").Map.Turtle.Cursed:FindFirstChild("GoodScroll") then
			CheckPQuest = Com("F_","CDKQuest","Progress","Good")
	...
	elseif AutoBuddySword and game.Players.LocalPlayer.PlayerGui.Main.Timer.Visible == false and (game:GetService("Workspace").Enemies:FindFirstChild("Cake Queen [Lv. 2175] [Boss]") or game:GetService("ReplicatedStorage"):FindFirstChild("Cake Queen [Lv. 2175] [Boss]")) then
        StatusFarm:Set({
            ["Title"] = "Status Farm",
            ["Content"] = "Kill Cake Queen"
        })
		if game.Workspace.Enemies:FindFirstChild("Cake Queen [Lv. 2175] [Boss]") then
			for i,v in pairs(game.Workspace.Enemies:GetChildren()) do
	...
	elseif AutoSpikeyTrident and game.Players.LocalPlayer.PlayerGui.Main.Timer.Visible == false and (havemob("Cake Prince [Lv. 2300] [Raid Boss]")) then
        StatusFarm:Set({
            ["Title"] = "Status Farm",
            ["Content"] = "Kill Cake Prince"
        })
	...
	elseif AutoTushita and MyLevel >= 2000 and game.Players.LocalPlayer.PlayerGui.Main.Timer.Visible == false and ((CheckHakiColor("Winter Sky") and CheckHakiColor("Pure Red") and CheckHakiColor("Snow White") and inmyself("God's Chalice")) or inmyself("Holy Torch") or (not game:GetService("Workspace").Map.Turtle:FindFirstChild("TushitaGate") and havemob("Longma [Lv. 2000] [Boss]")) or havemob("rip_indra True Form [Lv. 5000] [Raid Boss]") or (CheckButtonColorOpen() and inmyself("God's Chalice"))) then
        StatusFarm:Set({
            ["Title"] = "Status Farm",
            ["Content"] = "Do Tushita"
        })
	...
	elseif (AutoYama or AutoCDK) and Com("F_","EliteHunter", "Progress") >= 30 then
        StatusFarm:Set({
            ["Title"] = "Status Farm",
            ["Content"] = "Do Yama"
        })
		fireclickdetector(game.Workspace.Map.Waterfall.SealedKatana.Handle.ClickDetector)
	elseif AutoCanvander and game.Players.LocalPlayer.PlayerGui.Main.Timer.Visible == false and (game:GetService("Workspace").Enemies:FindFirstChild("Beautiful Pirate [Lv. 1950] [Boss]") or game:GetService("ReplicatedStorage"):FindFirstChild("Beautiful Pirate [Lv. 1950] [Boss]")) then
        StatusFarm:Set({
            ["Title"] = "Status Farm",
            ["Content"] = "Kill Beautiful Pirate"
        })
		if game.Workspace.Enemies:FindFirstChild("Beautiful Pirate [Lv. 1950] [Boss]") then
			for i,v in pairs(game.Workspace.Enemies:GetChildren()) do
	...
	elseif MyLevel >= 2300 and AutoSoulGuitar and ((Com("F_","GuitarPuzzleProgress","Check") ~= nil and Com("F_","GuitarPuzzleProgress","Check").Pipes == false) or (Com("F_","GuitarPuzzleProgress","Check") == nil and game:GetService("Lighting"):GetAttribute("MoonPhase") == 5 and CheckNight())) then
        StatusFarm:Set({
            ["Title"] = "Status Farm",
            ["Content"] = "Do Soul Gitar"
        })
		...
		if StartKaiTun then
			xpcall(function()
				...
				if (GodhumanDone or not AutoGodhuman) and (DeathStepDone or not AutoDeathStep) and (SharkmanKarateDone or not AutoSharkmanKarate) and (SuperhumanDone or not AutoSuperhuman) and (ElectricClawDone or not AutoElectricClaw) and (DargonTalonDone or not AutoDargonTalon) then
					...
					elseif AutoFarmMasterySword and #ThisSwordFarm > 0 and StatusFarm:Set({["Title"]="Status Farm",["Content"]="Do CDK"}) then                    

						while #ThisSwordFarm > 0 and StatusFarm:Set({["Title"]="Status Farm",["Content"]="Do CDK"}) and AutoFarmMasterySword do task.wait()
							for i,v in pairs(ThisSwordFarm) do
								if StatusFarm:Set({["Title"]="Status Farm",["Content"]="Do CDK"}) and AutoFarmMasterySword then
									TypeWeapon = "Sword"
									while StatusFarm:Set({["Title"]="Status Farm",["Content"]="Do CDK"}) and AutoFarmMasterySword do task.wait()
		...
		while task.wait(1) do 
			if StartKaiTun then
				xpcall(function()
					...
					if (Com("F_","GuitarPuzzleProgress","Check") ~= nil and Com("F_","GuitarPuzzleProgress","Check").Pipes == true) and ThreeWorld then
...
LPH_NO_VIRTUALIZE(function()
			...
			if StartKaiTun and CheckCanTeleport() and NearPlayer == true then
				...
				repeat task.wait(0.5)
					...
					if CheckError >= 500 then
						if (ThreeWorld) or not ThreeWorld then
	...
	spawn(function()
		while task.wait() do 
			if StartKaiTun and StatusFarm:Set({["Title"]="Status Farm",["Content"]="Auto Farm"})
			 then
				local CheckError = 0
				local RealError = 0
